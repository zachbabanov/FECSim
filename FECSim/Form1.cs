...
namespace FECSim
{
    public partial class Form1 : Form
    {
        ...
        private void sendFileButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                packetsRecieved = packetsRecieved.OrderBy(x => x.Key).ToDictionary(x => x.Key, x => x.Value);
            }
            catch (IndexOutOfRangeException){ }
            ...
            byte[] decodedData = decoder.Decode(recievedSequences);
            Debug.WriteLine("Encoded data decoded");

            File.WriteAllBytes(directoryName + "\\" + choosedFileName, decodedData);
            Debug.WriteLine("Recieved data written to file at specified path");
        }
    }
}