namespace FECSim
{
    public class Decoder
    {
        private static readonly byte[] epsilonTable = {
            
        };

        public byte[,]? decodedData;

        public Decoder()
        {
            decodedData = null;
        }

        public Decoder(int packetLenght)
        {
            decodedData = new byte[8, packetLenght];
        }

        public void Decode(byte[,] allRecievedBytes)
        {
            if (decodedData == null)
                throw new NullReferenceException();
            for (int currentOctet = 0; currentOctet < allRecievedBytes.GetLength(0); currentOctet++)
                for (int currentByte = 0; currentByte < 8; currentByte++)
                    decodedData[currentOctet, currentByte] = (byte)(allRecievedBytes[currentOctet, currentByte] 
                        ^ epsilonTable[(int)Math.Pow(2, currentByte) % 256]);
        }
    }
}
